...
(function ($) {
  ...
  $.fn.SetBubblePopup = function (options) {
    var _ = {	objTags: ['object', 'embed', 'applet'],
          ...
          me: this,
          markup: '\
              <table class="jqBubble" width="{TABLE_WIDTH}" cellpadding="0" cellspacing="0" style="{IE_FILTER_SHADOW}display:none; z-index:{Z-INDEX}; position:absolute; border:0px; border-collapse:collapse;"> \
              <tbody> \
              <tr> \
    ...
    function openBubble(popup_obj, tag_obj) {
      ...
      if (_.loading[i] || _.loaded[i]){
        ...
        var coord = new Array();
        coord["top"] = 0-parseInt(popup_obj.height())-parseInt(distanceFromTarget/2); //alert();
        coord["left"] = (bubbleAlign=='left' || bubbleAlign=='right' ? ( bubbleAlign=='left' ? 0-8 : parseInt(tag_obj.width())-parseInt(popup_obj.width())+18 ) : ( ((parseInt(tag_obj.width())-parseInt(popup_obj.width())+18)/2)) );
        popup_obj.css({
          top: coord["top"],
          left: coord["left"],
          position: 'absolute',
